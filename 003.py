...
from typing import TypedDict # Type disambiguation
...
class LLMMessage(TypedDict):  # Purpose passed to LLM API
    role: str
    content: str
...
async def call_llm(history: list[Message], llm_model: str) -> str: # not stream model
    try:
...
async def call_llm_stream(history: list[Message], llm_model: str, session_id: str) -> AsyncGenerator[str, None]: # stream model
    try:
